...
This box had very little SQL injection filtering, so we were able to use UNION SELECT and group(concat) to get the names of the SQL table and columns. We were also able to get a flag value from the website that allowed us to SSH onto the machine, but in hindsight we didn't even need to SSH since we could just look at the firewall.php file through the LOAD_FILE injection and deduce that we can use the HTTP header X-FORWARDED-FOR to run a bash reverse shell to the www-data user which had root privileges.